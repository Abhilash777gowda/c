...
def load_data():
    if os.path.exists("data/labeled_news.csv"):
        ...
        # Validate schema — discard if it was generated by the old synthetic pipeline
        # (which used different category names like 'assault', 'drug_crime', etc.)
        expected_cols = {'murder', 'rape', 'kidnapping', 'theft', 'accident', 'non_crime'}
        if not expected_cols.issubset(set(df.columns)):
            logger.warning(
                "Stale labeled_news.csv detected (wrong category columns). "
                "Discarding — click 'Fetch Live News' to regenerate."
            )
            return None
        return df
    return None
...
def fetch_live_news(max_per_feed: int = 20, skip_classification: bool = False):
    ...
    if skip_classification:
        ...
        logger.info("Skipping classification (Lite Mode active).")
    else:
        # Heuristic is vectorised and completes in milliseconds — classify ALL articles
